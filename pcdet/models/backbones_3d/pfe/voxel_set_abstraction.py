...
class MultiScaleVSA(nn.Module):
    
    def __init__(self, model_cfg, num_voxel_features=None, **kwargs):
        super().__init__()
        assert num_voxel_features is not None
        self.model_cfg = model_cfg
        
        hidden_dim = self.model_cfg.NUM_OUTPUT_FEATURES

        self.voxel_feature_transforms = nn.ModuleList()
        for src_name in self.model_cfg.FEATURES_SOURCE:
            src_ch = num_voxel_features[src_name]
            cur_layer = nn.Sequential(
                nn.Conv1d(src_ch, hidden_dim, kernel_size=1, bias=False),
                nn.BatchNorm1d(hidden_dim),
                nn.ReLU(),
            )
            self.voxel_feature_transforms.append(cur_layer)

        self.num_point_features_before_fusion = src_ch

        self.num_point_features = {'x_conv1': hidden_dim,
                                   'x_conv2': hidden_dim,
                                   'x_conv3': hidden_dim,
                                   'x_conv4': hidden_dim,
                                   }
    ...
    def forward(self, batch_dict):
        ...
        for k, src_name in enumerate(self.model_cfg.FEATURES_SOURCE):
            src_sp_tensor = batch_dict['multi_scale_3d_features'][src_name]
            src_features = src_sp_tensor.features
            # src_features: (1, C, N)
            src_features = src_features.unsqueeze(0).permute(0, 2, 1)
            point_features = self.voxel_feature_transforms[k](src_features)
            point_features = point_features.squeeze(0).transpose(0, 1)

            point_features = spconv.SparseConvTensor(
                            features=point_features.contiguous(),
                            indices=src_sp_tensor.indices,
                            spatial_shape=src_sp_tensor.spatial_shape,
                            batch_size=batch_dict['batch_size']
                        )

            point_features_dict.update({
                src_name: point_features
            })

        batch_dict['multi_scale_3d_features_post'] = point_features_dict
        batch_dict['with_voxel_feature_transform'] = True
        
        batch_dict['multi_scale_3d_strides'] =  {'x_conv1': 1,
                                                 'x_conv2': 2,
                                                 'x_conv3': 4,
                                                 'x_conv4': 8,
                                                }
